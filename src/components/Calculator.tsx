...
export const Calculator = () => {
  ...
  const calculateLoan = () => {
    ...
    if (principal && months && monthlyRate) {
      ...
      // Переключаемся на вкладку заявки после расчёта
    ...
    <div className="w-full max-w-2xl mx-auto">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-6">
          ...
          <TabsTrigger value="application" className="flex items-center gap-2" disabled={!result}>
            <Icon name="FileText" size={16} />
            Подача заявки
            {result && <span className="ml-1 w-2 h-2 bg-coral rounded-full"></span>}
          </TabsTrigger>
        </TabsList>
